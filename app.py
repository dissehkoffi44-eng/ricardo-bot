...
# Profils psychocoustiques affinés
PROFILES = {
    "krumhansl": {
...
def solve_key_logic(chroma_vector):
    """Analyse avec pondération de la tonique pour éviter les erreurs Fa/Sol/Do."""
    # Amplification des dominantes (non-linéaire)
    ...
    for p_name in ["bellman", "krumhansl"]:
        ...
        for mode in ["major", "minor"]:
            for i in range(12):
                # Rotation du profil pour la note i
                rotated_profile = np.roll(p_data[mode], i)
                
                # Corrélation de base
                score = np.corrcoef(cv, rotated_profile)[0, 1]
                
                # FACTEUR INTELLIGENT : Si la note i (tonique) est forte dans le signal,
                # on augmente artificiellement le score de cette tonalité.
...
def generate_reference_chord(key_str, duration=2.5, sr=22050):
    ...
    # Enveloppe ADSR simple (Fade out)
...
def process_audio(file_buffer, file_name, progress_bar, status_text):
    try:
        ...
        # 1. Séparation Harmonique/Percussive
        y_harmonic = librosa.effects.harmonic(y, margin=3.0)
        tuning = librosa.estimate_tuning(y=y_harmonic, sr=sr)
        
        # 2. Chroma CQT haute résolution (bins_per_octave=24 pour plus de précision)
        status_text.text("Analyse de l'empreinte harmonique...")
        chroma = librosa.feature.chroma_cqt(y=y_harmonic, sr=sr, tuning=tuning, 
                                            n_chroma=12, bins_per_octave=24)
        
        # 3. FILTRE MÉDIAN : Nettoyage des accords de passage (Fa -> Sol -> Do)
        # On lisse sur environ 2 secondes pour ne garder que la tonalité structurelle
        chroma_smooth = scipy.ndimage.median_filter(chroma, size=(1, 41))
        
        # 4. Score de confiance et calcul de la clé
        full_chroma_avg = np.mean(chroma_smooth, axis=1)
        res_logic = solve_key_logic(full_chroma_avg)
        
        # 5. Tempo
        status_text.text("Calcul du BPM...")
...
files = st.file_uploader("📂 Déposez vos tracks pour analyse experte", type=['mp3','wav','flac'], accept_multiple_files=True)
...
if files:
    for f in files:
        ...
        with st.expander(f"✅ RÉSULTAT : {res['name']}", expanded=True):
            ...
            with c1:
                # Design de la carte résultat
                color = "#10b981" if res['fiabilite'] > 70 else "#f59e0b"
                st.markdown(f"""
                    <div style="background:#0f172a; padding:20px; border-radius:15px; border-left: 8px solid {color}; color:white;">
                        <p style="margin:0; opacity:0.7;">TONALITÉ DÉTECTÉE</p>
                        <h1 style="margin:0; color:{color}; font-size:45px;">{res['key'].upper()}</h1>
                        <hr style="opacity:0.2;">
                        <div style="display:flex; justify-content:space-between;">
                            <span><b>CAMELOT:</b> {res['camelot']}</span>
                            <span><b>BPM:</b> {res['tempo']}</span>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
                
                st.write("#### 🎹 Test de cohérence")
                chord = generate_reference_chord(res['key'], sr=res['sr'])
                st.audio(chord, sample_rate=res['sr'])
                st.progress(res['fiabilite']/100, text=f"Indice de fiabilité : {res['fiabilite']}%")

            with c2:
                # Radar Chart du Cercle des Quintes
                # fig = go.Figure(data=go.Scatterpolar(
                    r=np.append(res['chroma_vals'], res['chroma_vals'][0]), 
                    theta=NOTES_LIST + [NOTES_LIST[0]], 
                    fill='toself',
                    line_color=color,
                    marker=dict(size=8)
                ))
                fig.update_layout(
                    ...
                    template="plotly_dark", height=350, margin=dict(l=30,r=30,t=30,b=30)
                )
                st.plotly_chart(fig, use_container_width=True)

st.sidebar.info("La version v2 utilise un filtrage médian et une pondération de la tonique pour distinguer les accords dominants des accords de passage.")
